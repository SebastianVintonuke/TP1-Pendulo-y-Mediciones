...
\todo{Terminar}
\textbf{Resultado de $g$ y Comparación con Valor de Referencia:}
...
\begin{itemize}
%    \item \textbf{Error de reacción humana en el cronómetro:} principal error accidental. Se estima en $\pm$0,1--0,2 s por accionamiento.
%                  Al medir 20 oscilaciones se acumula $\sim$$\pm$0,3 s en $t_{20}$, equivalente a $\Delta T \approx \pm 0{,}015$ s en $T$.
...
%    \item \textbf{Ordenada al origen no nula ($b \approx -0{,}07$ s$^2$):} el modelo ideal predice $b = 0$.
%                  El valor negativo puede deberse a errores sistemáticos en la medición de $L$ o del tiempo.
\end{itemize}

\subsubsection{Práctica 2 - Conclusiones}

%Para el péndulo, se verificó experimentalmente que el período es independiente de la amplitud para los ángulos ensayados ($\leq 20°$), en acuerdo con el modelo del péndulo ideal.
%La relación lineal entre $T^2$ y $L$ se confirmó con $r^2 = 0{,}9996$.
%El valor de $g$ obtenido, $(9{,}6 \pm 0{,}1)$ m/s$^2$, presenta una discrepancia de $\sim$2,4\% respecto al valor de referencia local, atribuible principalmente a la incerteza en la medición de la longitud efectiva del péndulo y al error de reacción del cronómetro.

Ambas prácticas ilustraron la importancia de la propagación correcta de incertezas, la elección del valor representativo, y la expresión de los resultados como $valor \pm incerteza [unidades]$.
