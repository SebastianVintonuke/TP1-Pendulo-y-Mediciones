...
Los tres intervalos tienen intersección no nula, todos contienen el intervalo $[8{,}285; 8{,}400]$ $\frac{g}{cm^3}$.
La regla y la probeta tienen intervalos mucho más amplios debido a la menor precisión de esos instrumentos.

\textbf{Comparación con materiales de referencia:} La densidad obtenida con el calibre (método más preciso) es $\rho = 8{,}342 \pm 0{,}057 \frac{g}{cm^3}$.
...
Tras realizar un ajuste con 12 puntos (correspondientes a $\theta$ = 5$^{\circ}$, 9$^{\circ}$ y 15$^{\circ}$), y aplicando el error de la pendiente del ajuste, se obtuvo como resultado $g = 9{,}58 \pm 0{,}26 \frac{m}{s^2}$.

\textbf{Valor de referencia en Buenos Aires:} $g_{\text{ref}} = 9{,}79 \frac{m}{s^2}$.

El valor de referencia se encuentra dentro del intervalo de indeterminación del resultado obtenido ($9{,}32 \frac{m}{s^2}$, $9{,}84 \frac{m}{s^2}$) lo que indica que el resultado es compatible con el valor de referencia.
...
\begin{itemize}
    ...
    \item \textbf{Amplitudes grandes:} Para ángulos mayores a 5$^{\circ}$ (especialmente el de 15$^{\circ}$ incluido en el cálculo final), la aproximación $\sen \theta \approx \theta$ pierde validez, introduciendo un error sistemático.
\end{itemize}
